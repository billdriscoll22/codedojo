...
function SAFE_HEAP_ACCESS(dest, type, store, ignore, storeValue) {
  ...
  assert(ignore || dest < Math.max(DYNAMICTOP, STATICTOP), 'segmentation fault');
  assert(ignore || DYNAMICTOP <= TOTAL_MEMORY, 'segmentation fault');
...
function setValue(ptr, value, type, noSafe) {
  ...
  if (noSafe) {
    ...
    switch(type) {
      ...
      case 'i64': (tempI64 = [value>>>0,Math.min(Math.floor((value)/4294967296), 4294967295)>>>0],SAFE_HEAP_STORE(ptr, tempI64[0], "i32", 0),SAFE_HEAP_STORE((ptr)+(4), tempI64[1], "i32", 0)); break;
...
/* memory initializer */ allocate([102,105,98,40,37,100,41,32,61,32,37,100,10,0,0,0,67,97,110,110,111,116,32,99,97,108,99,117,108,97,116,101,32,102,105,98,40,37,100,41,33,10,0,0,0,0,0,0,37,100,0,0,0,0,0,0,69,110,116,101,114,32,110,32,116,111,32,99,97,108,99,117,108,97,116,101,32,102,105,98,40,110,41,58,32,0,0,0], "i8", ALLOC_NONE, Runtime.GLOBAL_BASE)
...
function copyTempDouble(ptr) {
  ...
  function ___setErrNo(value) {
      ...
      SAFE_HEAP_STORE(___setErrNo.ret, value, "i32", 0)
      return value;
    }
  ...
  var __impure_ptr=allocate(1, "i32*", ALLOC_STATIC);var FS={currentPath:"/",nextInode:2,streams:[null],checkStreams:function () {
        ...
        SAFE_HEAP_STORE(_stdin, 1, "void*", 0);
        SAFE_HEAP_STORE(_stdout, 2, "void*", 0);
        SAFE_HEAP_STORE(_stderr, 3, "void*", 0);
        // Other system paths
  ...
  function _pwrite(fildes, buf, nbyte, offset) {
      ...
      if (!stream || stream.object.isDevice) {
        ...
        for (var i = 0; i < nbyte; i++) {
          contents[offset + i] = SAFE_HEAP_LOAD((buf)+(i), "i8", 1, 0);
        }
      ...
      if (!stream) {
        ...
        if (stream.object.isDevice) {
          if (stream.object.output) {
            for (var i = 0; i < nbyte; i++) {
              try {
                stream.object.output(SAFE_HEAP_LOAD((buf)+(i), "i8", 0, 0));
              } catch (e) {
          ...
          switch (type) {
            case 'd': case 'u': case 'i':
              if (half) {
                ...
                (tempI64 = [parseInt(text, 10)>>>0,Math.min(Math.floor((parseInt(text, 10))/4294967296), 4294967295)>>>0],SAFE_HEAP_STORE(argPtr, tempI64[0], "i32", 0),SAFE_HEAP_STORE((argPtr)+(4), tempI64[1], "i32", 0));
              } else {
  ...
  function _pread(fildes, buf, nbyte, offset) {
      ...
      if (!stream || stream.object.isDevice) {
        ...
        while (stream.ungotten.length && nbyte > 0) {
          SAFE_HEAP_STORE(buf++, stream.ungotten.pop(), "i8", 0)
          nbyte--;
          bytesRead++;
        }
        var contents = stream.object.contents;
        ...
        if (contents.slice) { // normal array
          for (var i = 0; i < size; i++) {
            SAFE_HEAP_STORE((buf)+(i), contents[offset + i], "i8", 0)
          }
        } else {
          for (var i = 0; i < size; i++) { // LazyUint8Array from sync binary XHR
            SAFE_HEAP_STORE((buf)+(i), contents.get(offset + i), "i8", 0)
          }
        }
        bytesRead += size;
        return bytesRead;
        ...
        if (stream.object.isDevice) {
          if (stream.object.input) {
            ...
            for (var i = 0; i < nbyte; i++) {
              ...
              bytesRead++;
              SAFE_HEAP_STORE((buf)+(i), result, "i8", 0)
            }
            return bytesRead;
          } else {
            ___setErrNo(ERRNO_CODES.ENXIO);
  ...
  function _ungetc(c, stream) {
      ...
      // http://pubs.opengroup.org/onlinepubs/000095399/functions/scanf.html
      var stdin = SAFE_HEAP_LOAD(_stdin, "void*", 0, 0);
...
function _main($argc, $argv) {
  ...
  while(1) switch(label) {
    case 1: 
      ...
      $2=$argc;
      $3=$argv;
      var $4=_printf(((64)|0), (tempInt=STACKTOP,STACKTOP = (STACKTOP + 1)|0,STACKTOP = ((((STACKTOP)+7)>>3)<<3),assert((STACKTOP|0) < (STACK_MAX|0)),SAFE_HEAP_STORE(tempInt, 0, "i32", 0),tempInt));
      var $5=_scanf(((56)|0), (tempInt=STACKTOP,STACKTOP = (STACKTOP + 8)|0,assert((STACKTOP|0) < (STACK_MAX|0)),SAFE_HEAP_STORE(tempInt, $n, "i32*", 0),tempInt));
      var $6=SAFE_HEAP_LOAD($n, "i32", 0, 0);
      var $7=(($6)|(0)) < 0;
      if ($7) { label = 2; break; } else { label = 3; break; }
    case 2: 
      var $9=SAFE_HEAP_LOAD(_stderr, "%struct.__sFILE*", 0, 0);
      var $10=SAFE_HEAP_LOAD($n, "i32", 0, 0);
      var $11=_fprintf($9, ((24)|0), (tempInt=STACKTOP,STACKTOP = (STACKTOP + 8)|0,assert((STACKTOP|0) < (STACK_MAX|0)),SAFE_HEAP_STORE(tempInt, $10, "i32", 0),tempInt));
      label = 4; break;
    case 3: 
      var $13=SAFE_HEAP_LOAD($n, "i32", 0, 0);
      var $14=SAFE_HEAP_LOAD($n, "i32", 0, 0);
      var $15=_fib($14);
      var $16=_printf(((8)|0), (tempInt=STACKTOP,STACKTOP = (STACKTOP + 16)|0,assert((STACKTOP|0) < (STACK_MAX|0)),SAFE_HEAP_STORE(tempInt, $13, "i32", 0),SAFE_HEAP_STORE((tempInt)+(8), $15, "i32", 0),tempInt));
      label = 4; break;
    case 4: 
      var i = __stackBase__; while ((i|0) < (STACKTOP|0)) { SAFE_HEAP_CLEAR(i|0); i = (i+1)|0 }STACKTOP = __stackBase__;
      return 0;
    default: assert(0, "bad label: " + label);
  }
}
